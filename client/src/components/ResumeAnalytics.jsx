...
const TooltipContent = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#1a1a25] border border-white/[0.08] rounded-xl px-3 py-2 shadow-xl">
        <p className="text-white/60 text-xs mb-1">{label}</p>
        <p className="text-white text-sm font-semibold">{payload[0].value} view{payload[0].value !== 1 ? 's' : ''}</p>
      </div>
    );
  }
  return null;
    ...
    <div className="glass-card rounded-2xl p-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.label} className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4 hover:bg-white/[0.05] transition-colors">
      ...
      {viewHistory.length > 0 && (
        <>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={viewsOverTime}>
                    ...
                    <Area
                      ...
                      dot={false}
                      activeDot={{ r: 4, fill: "#8B5CF6", stroke: "#1a1a25", strokeWidth: 2 }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="rounded-xl bg-white/[0.02] border border-white/[0.06] p-4">
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={countryData} layout="vertical">
                    <XAxis type="number" tick={{ fill: "#ffffff30", fontSize: 11 }} axisLine={false} tickLine={false} allowDecimals={false} />
                    <YAxis type="category" dataKey="country" tick={{ fill: "#ffffff50", fontSize: 11 }} axisLine={false} tickLine={false} width={80} />
          ...
          {recentViews.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-white/70 mb-4">Recent Views</h3>
              <div className="space-y-1.5">
                {recentViews.map((view, i) => {
                  const date = new Date(view.date);
                  const timeAgo = getTimeAgo(date);
                  return (
                    <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.04] transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-white/80 font-medium truncate">{view.country || "Unknown"}</p>
      ...
      {viewHistory.length === 0 && (
        <div className="text-center py-12">
          <div className="inline-flex size-14 rounded-xl bg-white/[0.03] border border-white/[0.06] items-center justify-center mb-4">
            <Eye className="size-6 text-white/10" />
          </div>
          <p className="text-white/40 font-medium text-sm">No views yet</p>
