...
const Testimonial = () => {
  ...
  return (
    <>
      ...
      {loading ? (
        <div className="bg-[#08080f] flex items-center justify-center pb-28">
        ...
        <div className="bg-[#08080f] pb-28">
          <p className="text-center text-white/30 text-sm">No testimonials yet. Be the first!</p>
        </div>
      )}

      <div className="bg-[#08080f] pb-28 px-4">
        <div className="max-w-lg mx-auto">
          ...
          <form onSubmit={handleSubmit} className="bg-white/[0.04] backdrop-blur-xl border border-white/[0.06] rounded-2xl p-6 space-y-4">
            ...
            <div className="w-full bg-white/5 border border-white/10 rounded-xl overflow-hidden pl-4 pt-3 gap-2.5 focus-within:border-violet-500/50 focus-within:ring-2 focus-within:ring-violet-500/20 transition-all">
              <textarea name="text" placeholder="Your testimonial..." required maxLength={500} rows={4}
                className="border-none outline-none ring-0 bg-transparent w-full text-white/90 placeholder:text-white/30 resize-none"
                value={form.text}
                onChange={(e) => setForm(p => ({ ...p, text: e.target.value }))}
              />
              <div className="text-right text-xs text-white/20 pb-2 pr-4">{form.text.length}/500</div>
            </div>
            <div className="flex items-center gap-2 text-sm text-white/50">
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Star className={`size-5 ${star <= form.rating ? 'fill-violet-500 text-violet-500' : 'text-white/20'}`} />
                </button>
              ))}
            </div>
